...
## 3. 初始化命令行配置文件
在命令行终端输入如下命令，[更多命令](https://github.com/i18n-pro/core/blob/v3.0.0/docs/dist/COMMAND_LINE_zh-CN.md#命令列表)
```bash
npx i18n init 
```
...
## 4. 调整 `i18nrc.ts` 配置
根据需求自行调整配置文件中的配置项，配置项的[说明](https://github.com/i18n-pro/core/blob/v3.0.0/docs/dist/COMMAND_LINE_zh-CN.md#1--i18nrcts-配置)
...
> 推荐在每个语言单独文件形式时使用此方式，可实现按需异步加载，减少首屏体积，提升页面性能<br/>
```js
// i18n.js
import { initI18n } from 'i18n-pro'
import ja from './i18n/ja.json'
...
> 适用于需要在运行时动态获取语言包的场景，例如通过远程接口拉取语言包数据，适合多端或动态语言环境<br/>
...
* 真实示例请参考  [Live Demo](https://github.com/i18n-pro/core/blob/v3.0.0/README_zh-CN.md#live-demo) 
